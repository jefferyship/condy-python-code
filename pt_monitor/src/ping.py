...
def pingNode(alive=0, timeout=1.0, ipv6=0, number=sys.maxint, node=None, \
    ...
    if not alive:
        if ipv6:
            ...
            if noPrintIPv6adr == 1:
                ...
                _writeLog( "PING %s : %d data bytes (40+8+%d)" % (str(node), \
                                                             40+8+size, size))
            else:
                # add 40 (header) + 8 (icmp header) + payload
                _writeLog( "PING %s (%s): %d data bytes (40+8+%d)" % (str(node), \
                                                                 str(host), 40+8+size, size))
        else:
            # add 20 (header) + 8 (icmp header) + payload
            _writeLog( "PING %s (%s): %d data bytes (20+8+%d)" % (str(node), str(host), \
                                                             20+8+size, size))
    ...
    try:
        ...
        while start <= number:
            ...
            if not pong:
                if alive:
                    _writeLog( "no reply from %s (%s)" % (str(node), str(host)))
                else:
                    _writeLog( "ping timeout: %s (icmp_seq=%d) " % (host, start))
            ...
            if alive:
                _writeLog( str(node) + " (" + str(host) +") is alive")
            else:
                if ipv6:
                    # size + 8 = payload + header
                    _writeLog( "%d bytes from %s: icmp_seq=%d time=%.5f ms" % \
                          (size+8, host, pongSeqnr, triptime*1000))
                else:
                    _writeLog( "%d bytes from %s: icmp_seq=%d ttl=%s time=%.5f ms" % \
    ...
    if start != 0 or lost >0:  # do not print stats if 0 packet sent
        ...
        if not alive:
            ...
            _writeLog( "%d packets transmitted, %d packets received, %d%% packet loss" % \
                  (start, start-lost, plost))
            # don't display summary if 100% packet-loss
            if plost != 100:
                _writeLog( "round-trip min/avg/max/stddev = %.3f/%.3f/%.3f/%.3f ms" % \
                      (mint*1000, (tsum/start)*1000, maxt*1000, math.sqrt(vari)*1000))

    pingSocket.close()
    return (start,start-lost,plost)
...
def _error(err):
    ...
    if __name__ == '__main__':
        _writeLog( "%s: %s" % (os.path.basename(sys.argv[0]), str(err)))
        _writeLog( "Try `%s --help' for more information." % os.path.basename(sys.argv[0]))
        sys.exit(1)
    else:
...
def ping(node,count=5,size=ICMP_DATA_STR):
    """
      @param count:ping的次数
      @param node:ping的IP地址
      @param size:ping包的大小
      @return (totalPacket,recivePacket,lostPecent) (总报数,接收到包数,包的丢失率)
    """
    alive = 0; timeout = 1.0; ipv6 = 0;
    flood = 0;
    return pingNode(alive=alive, timeout=timeout, ipv6=ipv6, number=count, \
