...
class dm_term_call_log(object):
    ...
    def set_finish_reason(self,cc_agentcalldetail):
        ...
        99 其他原因
        """
        if cc_agentcalldetail.callendtype ==1:#用户挂机
           if  cc_agentcalldetail.answertime==None:#并且应答时间空，说明通话还没有建立，用户主动挂机
               self.finish_reason=5
           else: #并且应答时间不为空，说明呼入成功
               self.finish_reason=2
        elif cc_agentcalldetail.callendtype ==2:#客代挂机
           if  cc_agentcalldetail.answertime==None:#并且应答时间空，说明通话还没有建立，客代主动挂机
